...
open class TestServer {
    ...
    internal class TestServerEventHandler : TServerEventHandler {
        ...
        override fun deleteContext(
            ...
            val ctx: TestServerContext = serverContext.unwrap(TestServerContext::class.java)
            println(
...
fun main(args: Array<String>) {
    try {
        ...
        try {
            for (i in args.indices) {
                if (args[i].startsWith("--port")) {
                    ...
                    stringLimit = Integer.valueOf(args[i].split("=").toTypedArray()[1]).toLong()
                } else if (args[i].startsWith("--container-limit")) {
                    containerLimit = Integer.valueOf(args[i].split("=").toTypedArray()[1]).toLong()
                } else if (args[i] == "--help") {
        ...
        try {
            checkServerType(serverType, ssl)
        ...
        // Protocol factory
        val tProtocolFactory = getTProtocolFactory(protocolType, stringLimit, containerLimit)
        val tTransportFactory = getTTransportFactory(transportType)
        val serverEngine =
            getTServer(
                testProcessor,
                secondProcessor,
...
private fun checkTransportType(transportType: String) {
    when (transportType) {
        "buffered" -> {}
        "framed" -> {}
        "fastframed" -> {}
...
private fun checkProtocolType(protocolType: String) {
    when (protocolType) {
        "binary" -> {}
        "compact" -> {}
        "json" -> {}
...
private fun checkServerType(serverType: String, ssl: Boolean) {
    when (serverType) {
        "simple" -> {}
        "thread-pool" -> {}
        "nonblocking" -> {
    ...
    when (serverType) {
        "nonblocking", "threaded-selector" -> {
            ...
            if (serverType.contains("nonblocking")) {
                // Nonblocking Server
                val tNonblockingServerArgs: TNonblockingServer.Args =
                ...
                val tThreadedSelectorServerArgs: TThreadedSelectorServer.Args =
            ...
            // SSL socket
            var tServerSocket: TServerSocket? = null
            if (ssl) {
                tServerSocket = TSSLTransportFactory.getServerSocket(port, 0)
            } else {
                tServerSocket = TServerSocket(TServerSocket.ServerSocketTransportArgs().port(port))
            }
            if (serverType == "simple") {
                // Simple Server
                val tServerArgs: TServer.Args = TServer.Args(tServerSocket)
                ...
                // ThreadPool Server
                val tThreadPoolServerArgs: TThreadPoolServer.Args =
...
private fun getTTransportFactory(transportType: String) =
    when (transportType) {
        "framed" -> {
...
private fun getTProtocolFactory(protocolType: String, stringLimit: Long, containerLimit: Long) =
    when (protocolType) {
        "json", "multij" -> {
